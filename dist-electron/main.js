  ...
  try {
    ...
    for (const migration of pendingMigrations) {
      try {
        ...
        console.error(`マイグレーション version ${migration.version} の適用に失敗しました:`, error2);
        throw error2;
      }
    }
    console.log("マイグレーションが完了しました。");
  } catch (error2) {
    console.error("マイグレーション中にエラーが発生しました:", error2);
    throw error2;
  }
}
function validateFilePath(filePath, allowedDirectories) {
  try {
    ...
    console.error("Error validating file path:", error2);
    return false;
  }
}
  ...
  let NsisUpdater$1 = class NsisUpdater extends BaseUpdater_1.BaseUpdater {
    ...
    doInstall(options) {
      ...
      const callUsingElevation = () => {
        this.spawnLog(path$1.join(process.resourcesPath, "elevate.exe"), [options.installerPath].concat(args)).catch((e) => this.dispatchError(e));
      };
      ...
      this.spawnLog(options.installerPath, args).catch((e) => {
        ...
        if (errorCode === "UNKNOWN" || errorCode === "EACCES") {
          ...
          require$$1$3.shell.openPath(options.installerPath).catch((err) => this.dispatchError(err));
        } else {
          this.dispatchError(e);
        }
      });
...
class UpdateHandler {
  ...
  initialize() {
    ...
    require$$1$3.ipcMain.handle("start-update", () => {
...
require$$1$3.app.setName(APP_NAME);
if (process.platform === "darwin") {
  try {
    require$$1$3.app.setName(APP_NAME);
    require$$1$3.app.setAboutPanelOptions({
      applicationName: APP_NAME,
      applicationVersion: require$$1$3.app.getVersion(),
  ...
  try {
    require$$1$3.app.setAppUserModelId(`com.moritaniantech.${APP_NAME}`);
  } catch (e) {
    console.error("Failed to set app user model id:", e);
  }
}
const isDev = process.env.NODE_ENV === "development" || !require$$1$3.app.isPackaged;
const DIST_ELECTRON = path__namespace.join(__dirname, "../dist-electron");
const DIST = path__namespace.join(__dirname, "../dist");
require$$1$3.app.isPackaged ? DIST : path__namespace.join(__dirname, "../public");
...
async function initializeStore() {
  try {
    ...
    console.error("Error initializing store:", error2);
    return {
      get: () => "dark",
      set: () => {
      },
...
async function initializeDatabase() {
  const dbPath = path__namespace.join(require$$1$3.app.getPath("userData"), "slay-the-spire-stats.db");
  return new Promise((resolve, reject) => {
...
async function createWindow() {
  require$$1$3.session.defaultSession.webRequest.onHeadersReceived(
    (details, callback) => {
      callback({
        responseHeaders: {
  ...
  if (process.platform === "darwin") {
    require$$1$3.app.dock.setIcon(ICON);
  }
  if (isDev) {
  ...
  require$$1$3.session.defaultSession.setPermissionRequestHandler((webContents, permission, callback) => {
  ...
  require$$1$3.session.defaultSession.setPermissionCheckHandler((webContents, permission) => {
...
require$$1$3.app.whenReady().then(async () => {
  try {
    ...
    await createWindow();
    require$$1$3.app.on("activate", async () => {
      if (require$$1$3.BrowserWindow.getAllWindows().length === 0) {
        await createWindow();
      }
    });
  } catch (error2) {
    console.error("Application initialization error:", error2);
...
require$$1$3.ipcMain.handle("select-folder", async () => {
  const result = await require$$1$3.dialog.showOpenDialog({
    properties: ["openDirectory"]
  });
  return result.filePaths[0];
});
require$$1$3.ipcMain.handle("get-run-folder", async () => {
  return new Promise((resolve, reject) => {
    if (!db) {
...
require$$1$3.ipcMain.handle("load-run-files", async (_, folderPath) => {
  try {
    ...
    for (const character of characters) {
      ...
      const runFiles = files.filter((file2) => file2.endsWith(".run"));
      for (const file2 of runFiles) {
        const filePath = path__namespace.join(characterPath, file2);
    ...
    console.error("Error loading run files:", error2);
    throw error2;
  }
});
require$$1$3.ipcMain.handle("get-all-runs", async () => {
  return new Promise((resolve, reject) => {
    if (!db) {
      reject(new Error("Database not initialized"));
      return;
...
require$$1$3.ipcMain.handle("get-theme", async () => {
  return store.get("theme");
});
require$$1$3.ipcMain.handle("set-theme", async (_, theme) => {
  store.set("theme", theme);
  return theme;
});
require$$1$3.ipcMain.handle("export-data", async () => {
  if (!db) throw new Error("Database not initialized");
  const exportPath = path__namespace.join(require$$1$3.app.getPath("userData"), "backup.json");
  ...
  await safeWriteFile(exportPath, JSON.stringify(runs, null, 2), [require$$1$3.app.getPath("userData")]);
  return exportPath;
});
require$$1$3.ipcMain.handle("import-data", async (_, filePath) => {
  if (!db) throw new Error("Database not initialized");
...
require$$1$3.ipcMain.handle("delete-run", async (_, run) => {
  if (!db) throw new Error("Database not initialized");
  return new Promise((resolve, reject) => {
    db == null ? void 0 : db.run(
...
require$$1$3.ipcMain.handle("sqlite-execute", async (_, sql, params = []) => {
  if (!db) throw new Error("Database not initialized");
  return new Promise((resolve, reject) => {
...
require$$1$3.ipcMain.handle("sqlite-query", async (_, sql, params = []) => {
  if (!db) throw new Error("Database not initialized");
  return new Promise((resolve, reject) => {
...
require$$1$3.ipcMain.handle("sqlite-get", async (_, sql, params = []) => {
  if (!db) throw new Error("Database not initialized");
  return new Promise((resolve, reject) => {
...
async function backupDatabase() {
  if (!db) return;
  const backupDir = path__namespace.join(require$$1$3.app.getPath("userData"), "backups");
  if (!fs__namespace.existsSync(backupDir)) {
    fs__namespace.mkdirSync(backupDir);
  }
  const timestamp2 = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-");
  const backupPath = path__namespace.join(backupDir, `backup-${timestamp2}.json`);
  try {
    const runs = await new Promise((resolve, reject) => {
      db == null ? void 0 : db.all("SELECT * FROM runs", (err, rows) => {
        if (err) reject(err);
    ...
    if (backupFiles.length > MAX_BACKUPS) {
      for (const file2 of backupFiles.slice(MAX_BACKUPS)) {
        await fs__namespace.promises.unlink(path__namespace.join(backupDir, file2));
      }
    }
    console.log(`バックアップを作成しました: ${backupPath}`);
  } catch (error2) {
    console.error("バックアップ作成中にエラーが発生しました:", error2);
  }
}
let backupInterval;
...
require$$1$3.app.on("ready", () => {
  startBackupInterval();
});
require$$1$3.app.on("before-quit", () => {
  if (backupInterval) {
    clearInterval(backupInterval);
  }
});
require$$1$3.app.on("before-quit", async (event) => {
  event.preventDefault();
  try {
    await closeDatabase();
    require$$1$3.app.exit();
  } catch (error2) {
    console.error("Error during application shutdown:", error2);
    require$$1$3.app.exit(1);
  }
});
